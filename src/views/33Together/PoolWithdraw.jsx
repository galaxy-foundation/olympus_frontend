      ...
      {!address ? (
        ...
        <Box className="withdrawal-container">
          <Box display="flex" alignItems="center" flexDirection={`${isMobileScreen ? "column" : "row"}`}>
            ...
            >
              {exitFee > 0
                ? txnButtonText(pendingTransactions, "pool_withdraw", "Withdraw Early & pay " + exitFee + " sPIP")
                : txnButtonText(pendingTransactions, "pool_withdraw", "Withdraw sPIP")}
              {/* Withdraw sPIP */}
            </Button>
          </Box>
          {newOdds > 0 && quantity > 0 && (
            <Box padding={1}>
              <Typography color="error" variant="body2">
                Withdrawing {quantity} sPIP reduces your odds of winning to 1 in {newOdds}&nbsp;
              </Typography>
            </Box>
          )}
          {exitFee > 0 && (
          ...
          {/* NOTE (Appleseed): added this bc I kept losing track of which accounts I had sPIP in during testing */}
